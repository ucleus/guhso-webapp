...
@section('title', 'Create Advertisement - Guhso Dashboard')

@section('content')
<div class="mb-8">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 mb-2">Create Advertisement</h1>
            <p class="text-gray-600">Set up a new advertisement campaign</p>
        </div>
...
<form action="{{ route('dashboard.ads.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
    @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="duration_days" id="duration_days" value="{{ old('duration_days') }}" min="1" 
                       ...
                       placeholder="30">
                <p class="mt-1 text-xs text-gray-500">How many days should this ad run?</p>
                @error('duration_days')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div class="md:col-span-2">
                ...
                <p class="mt-1 text-xs text-gray-500">Inactive advertisements won't be displayed</p>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateEndDate() {
        if (startInput.value && durationInput.value) {
            ...
            endDateDisplay.textContent = `Advertisement will end on: ${endDate.toLocaleDateString()}`;
        }
    }
